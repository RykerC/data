...
def read_data(fname, source_count, source_word2idx, target_count, target_word2idx):
  ...
  for sentence in root:
    text = sentence.find('sentence').text.lower()
    fro, to = getlocation(text, sentence.iter('aspects_seed'))
    if len(text.strip()) != 0:
      idx = []
      ...
      for asp_terms in sentence.iter('aspects_seed'):
        for asp_term in asp_terms.findall('aspects_seed'):
          ...
          pos_info, lab = _get_data_tuple(text, asp_term.lower(), int(fro), int(to), sentence.iter('pol'), source_word2idx)
